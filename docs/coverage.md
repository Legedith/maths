...
7. Develop literature ingestion with licensing, identity resolution, provenance and benchmarked retrieval. Keep candidate links separate from established edges.

Human work is especially valuable in selecting a useful formulation, finding a counterexample to missing assumptions, explaining an entry, checking primary sources, and deciding whether an application model is faithful. Agents can automate validated finite calculations, consistency checks, source retrieval and candidate generation; their outputs still need explicit verification appropriate to the claim.
