...
def compile_index(source_root: Path) -> dict[str, dict]:
    ...
    parent_codes = {row["navigation_parent_code"] for row in subjects}
    for row in subjects:
        row["hierarchy_level"] = 1 if row["kind"] == "top_level" else (
            3 if row["kind"] == "subject_leaf" else 2)
        row["is_leaf"] = row["code"] not in parent_codes
