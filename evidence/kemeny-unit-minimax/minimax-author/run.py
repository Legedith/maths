...
a=['uv','run','--isolated','--python','3.12.11','python',str(p/'check.py')];start=time.monotonic()
...
(p/'stdout.bin').write_bytes(o);(p/'stderr.bin').write_bytes(e);(p/'run.json').write_text(json.dumps({'argv':a,'cwd':str(p),'rc':rc,'elapsed':time.monotonic()-start,'timeout':60},indent=2)+'\n',encoding='utf-8');print(o.decode());print(e.decode());raise SystemExit(rc)
