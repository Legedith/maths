...
function Provenance({ edge, observation }: { edge: ExternalEdge; observation?: RelationObservation }) {
  return <details className={styles.provenance}>
    ...
    {observation && <>
      <p>A separate response retrieved at {observation.retrieved_at_utc} is attached below. <a href={`https://www.wikidata.org/w/index.php?title=${edge.source.id}&oldid=${observation.lastrevid}`} target="_blank" rel="noreferrer">Entity revision {observation.lastrevid}</a>.</p>
...
function Neighborhood({ result, select }: { result: RelationResult; select: (qid: string) => void }) {
  ...
  const draw = (edge: ExternalEdge, index: number, left: boolean) => {
    ...
    return <g key={edge.id}>
      ...
      <g role="button" tabIndex={0} aria-label={`Explore ${neighbor.label}`} onClick={() => select(neighbor.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); select(neighbor.id); } }} className={styles.graphNode}>
  ...
  return <figure className={styles.neighborhood}>
    ...
    <div className={styles.diagramScroll}><svg viewBox={`0 0 940 ${height}`} role="group" aria-label={`Directed connections around ${result.concept.label_as_recorded}`}>
...
export function ExternalConnections({ initial }: { initial: RelationResult }) {
  ...
  const select = useCallback(async (input: RelationQuery, updateHistory = true): Promise<RelationResult> => {
    ...
    try {
      ...
      if (current === generation.current) { setLoading(false); setError('Could not open that concept or filter. Your previous selection is still shown.'); }
      throw cause;
    }
  }, [initial.availableProperties]);
  ...
  useEffect(() => {
    const restore = () => {
      ...
      try {
        ...
        void select(next, false).catch(() => undefined);
      } catch { setError('This link has invalid concept or filter values. The initial concept is shown.'); }
    };
    if (window.location.search) queueMicrotask(restore);
    window.addEventListener('popstate', restore);
    return () => { window.removeEventListener('popstate', restore); generation.current++; searchGeneration.current++; };
  }, [initial.qid, select]);
    ...
    <div className={styles.workspace}>
      ...
      <section className={styles.content} aria-busy={loading}>
        ...
        <div className={styles.filters}>
          ...
          <p role="status">{loading ? 'Opening connections…' : `${result.total} matching relations`}</p>
        </div>
