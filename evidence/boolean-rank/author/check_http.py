...
output = WORK / 'results/http-02.json'
if output.exists():
    raise FileExistsError(output)
expected = json.loads((PROJECT / 'data/atlas.json').read_bytes())
responses = []
with urlopen('http://localhost:8598/api/atlas', timeout=40) as response:
    body = response.read()
    ...
    (WORK / 'results/http-02-api.json').write_bytes(body)
with urlopen('http://localhost:8598/', timeout=40) as response:
    body = response.read()
    ...
    (WORK / 'results/http-02-home.html').write_bytes(body)
