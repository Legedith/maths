...
globalThis.__auditReactHooks = projectRequire('react');
globalThis.__auditReactDomHooks = projectRequire('react-dom');
...
const hooks = registerHooks({
  ...
  load(url, context, nextLoad) {
    if (url.startsWith(projectUrl) && /\.tsx?$/.test(url) && !url.includes('/node_modules/')) {
      ...
      if (url.endsWith('/lib/use-atlas-tools.ts')) {
        source = source
          .replace("import { useEffect } from 'react';", 'const { useEffect } = globalThis.__auditReactHooks;')
          .replace("import { flushSync } from 'react-dom';", 'const { flushSync } = globalThis.__auditReactDomHooks;');
      }
...
await check('live HTTP API and home response agree with frozen product', async () => {
  ...
  assert.ok(normalizedHome.includes('NETWORKS &amp; BOOLEAN RELATIONS'));
  assert.ok(normalizedHome.includes('40 concepts'));
  assert.ok(normalizedHome.includes('11 sources'));
