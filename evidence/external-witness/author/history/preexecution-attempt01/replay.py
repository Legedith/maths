...
def calculate(output):
    ...
    passed = (result_first["source_control_matches"] and result_first["summary_disagrees"] and result_first["basis_certificate_holds"] and result_second["cofactors_match_tau"] and summary_query["status"] == "sat" and source_query["status"] == "unsat" and replay is not None and all(replay[k] for k in ["node_equations_hold", "independent_solution_matches", "source_control_matches", "summary_disagrees"]))
    save(output / "result-summary.json", {"known_case_count": 2, "author_expected_checks_pass": bool(passed), "independent_gate": "pending", "scope": encoding["scope"], "encoding_sha256": hashlib.sha256(encoding_path.read_bytes()).hexdigest(), "versions": {"python": platform.python_version(), "z3-solver": version("z3-solver"), "sympy": version("sympy"), "mpmath": version("mpmath")}})
