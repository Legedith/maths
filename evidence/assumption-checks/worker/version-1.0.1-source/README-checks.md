...
Each evaluation trace contains its normalized AST path, child operands, exact value, type, shape, status, and any runtime error. The output also contains the normalized input and its SHA-256 hash, supplied provenance, all assumption records, the claim record, lazily accessed quantity records, certificates, errors, and `atlas-checks/1.0.1` as the algorithm version. Hash input is JSON with sorted keys, compact separators and ASCII escapes, encoded as ASCII/UTF-8. Escaped unpaired surrogates are retained as supplied strings without crashing the hash operation.
