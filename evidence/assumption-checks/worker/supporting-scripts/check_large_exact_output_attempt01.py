...
try:
    ...
    report.update({"escaped_exception": False, "verdict": result["verdict"]})
except Exception as exc:
    report.update({"escaped_exception": True, "exception_type": type(exc).__name__, "message": str(exc), "traceback": traceback.format_exc()})
(root / "evidence/assumption-checks/worker/root-large-output-attempt-01.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
print(json.dumps({k:v for k,v in report.items() if k not in ("input", "traceback")}, indent=2))
