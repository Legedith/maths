...
root=Path('D:/CodexWorkspaces/mathematics-atlas/project');bp=root/'.codex/evidence/runs/kemeny-uniform-repair-v1/bundle.json';b=json.loads(bp.read_text());arts={a['id']:a for a in b['artifacts']};errors=[]
def need(v,msg):
 if not v:errors.append(msg)
def decode(p):
 try:return p.read_text(encoding='utf-8-sig')
 except UnicodeDecodeError:return p.read_text(encoding='cp1252')
def read(p):return json.loads(decode(p))
...
manifest=read(root/'evidence/kemeny-uniform-repair/copy-manifest.json')
for row in manifest:
...
for c in b['claims']:
 ...
 for u in c['supports']:
  ...
  a=arts[u['artifact_id']];kinds.append(a['kind']);txt=decode(root/a['path']);loc=u['locator']
