...
def symbolic(case):
 ...
 out={'case':case,'q':[[str(t) for t in row] for row in q],'N':str(N),'D':str(D),'negative_numerator_terms':[[list(m),str(v)] for m,v in P.terms()],'denominator_terms':[[list(m),str(v)] for m,v in T.terms()],'numerator_positive':all(v>0 for v in P.coeffs()) and P.TC()>0,'denominator_positive':all(v>0 for v in T.coeffs()) and T.TC()>0,'numerator_constant':str(P.TC()),'denominator_constant':str(T.TC())}
