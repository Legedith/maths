...
for a,b,c in [(3,3,3),(3,4,5),(4,4,6),(3,3,7),(4,5,5)]:
 ...
 rows.append({'parts':[a,b,c],'deltas':{k:str(v) for k,v in out.items()},'order':sorted(out,key=out.get),'H1':out['restore']==min(out.values()),'H2':out['untouched_A']<out['uv'],'H3':out['uv']==min(v for k,v in out.items() if k!='restore')})
